...
export function MemoryMetricsChart({ data }: MemoryMetricsChartProps) {
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 backdrop-blur-md relative overflow-hidden shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">PromQL Container Memory Working Set</h3>
            <p className="text-[11px] text-slate-500">sum(container_memory_working_set_bytes)</p>
          </div>
        </div>

        <span className="text-[11px] font-mono font-semibold text-purple-500 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/30">
          Range: {data?.range || '1h'}
        </span>
      </div>
      ...
      <div className="relative w-full h-[180px]">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
          <defs>
            <linearGradient id="promMemoryGradient" x1="0" y1="0" x2="0" y2="1">
          ...
          {areaD && <path d={areaD} fill="url(#promMemoryGradient)" />}
        ...
        {hoveredPoint && (
          <div className="absolute top-2 right-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-2 rounded-lg text-xs font-mono shadow-md pointer-events-none">
            ...
            <span className="text-purple-500 font-bold">{hoveredPoint.value} GB</span>
          </div>
        )}
      </div>
